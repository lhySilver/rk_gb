...
def manual(): 
    print """
    usage:
        python slothxml.py [option] filelist
            [option]
                -f: use file in filelist
                -p: use dir in filelist, will be parsed recursive
                
    sample:
        python slothxml.py -f file1.json file2.json file3.json
        python slothxml.py -p ./dir1/ ./dir2/ ./dir3/
        """
...
def get_member_methods():
    ...
    __construct = {
        ...
        'imp': lambda ctype, members: merge([merge([
            ...
            '    __xml_has_%s = false;' % member[NAME],
    ...
    __encode = {
        'dec': lambda declare, ctype: '    bool encode(xml_document_t& doc, xml_element_t& xml_val) const;' if declare else (
            'bool %s::encode(xml_document_t& doc, xml_element_t& xml_val) const' % ctype
            ),
        'imp': lambda ctype, members: __do_while_frame(merge([
            '        if (!__skip_%s && !encode_field(%s, "%s", doc, xml_val)) break;' % (
                member[NAME], member[NAME], member[NAME]) for member in members
            ]))
        }
    __decode = {
        'dec': lambda declare, ctype: '    bool decode(const xml_element_t& xml_val);' if declare else (
            'bool %s::decode(const xml_element_t& xml_val)' % ctype
            ),
        'imp': lambda ctype, members: __do_while_frame(merge([
            '        if (!decode_field(xml_val, "%s", %s, __xml_has_%s)) break;' % (
                member[NAME], member[NAME], member[NAME]) for member in members
            ]))
        }
...
def get_object_methods():
    __encode = {
        'dec': lambda ctype: 'bool encode(const %s& obj_val, xml_document_t& doc, xml_element_t& xml_val)' % ctype,
        'imp': '    return obj_val.encode(doc, xml_val);'
        }
    __decode = {
        'dec': lambda ctype: 'bool decode(const xml_element_t& xml_val, %s& obj_val)' % ctype,
        'imp': '    return obj_val.decode(xml_val);'
        }
    ...
    def __gen_interface(path, obj, member_methods, object_methods):
        def __gen_struct(struct, member_methods, object_methods):
            __gen_def = lambda ctype, members: merge([
                ...
                '',
                merge(['    bool xml_has_%s() const { return __xml_has_%s; }' % (member[NAME], member[NAME]) for member in members]),
                '',
                ...
                '',
                merge(['    bool __xml_has_%s;' % member[NAME] for member in members]),
                '};',
                ''
                ])
            return merge([
