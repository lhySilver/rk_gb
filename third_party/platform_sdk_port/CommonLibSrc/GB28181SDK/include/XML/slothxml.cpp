...
namespace slothxml
{
  ...
  //      }
		//
  //      xml_attribute_t * attr = doc.allocate_attribute(val);
  //      if (!attr)
  //      {
  //          return false;
  //      }
		////xml_val.append_node()
		 ...
		 xml_element_t * node = doc.allocate_node(rapidxml::node_data,0,doc.allocate_string(str_val));
		 if(!node)
		 {
			 return false;
		 }
         xml_val.append_node(node);
        return true;
    }
    const char * get_attribute(const xml_element_t& xml_val, const char * key)
    {
		...
		return attr->value();*/
		xml_element_t * node = xml_val.first_node();
		if(!node)
		{
            return NULL;
		}
