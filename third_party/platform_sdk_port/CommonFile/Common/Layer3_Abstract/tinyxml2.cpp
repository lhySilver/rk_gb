...
bool XMLUtil::ToULongLong( const char* str, unsigned long long* value )
{
...
char* XMLText::ParseDeep( char* p, StrPair* )
{
    const char* start = p;
    if ( this->CData() ) {
		/** StrPair::NEEDS_NEWLINE_NORMALIZATION该属性在这里会把回车换行(\r\n)转换为linux的回车换行(\n)，
		** 在此不需要此属性，即：不需要对CData里面的字符进行转换*/
		//p = _value.ParseText( p, "]]>", StrPair::NEEDS_NEWLINE_NORMALIZATION );
		p = _value.ParseText( p, "]]>", 0);
        if ( !p ) {
...
XMLError XMLElement::QueryLongLongText( long long * llval ) const
{
    if ( FirstChild() && FirstChild()->ToText() ) {
        const char* t = FirstChild()->ToText()->Value();
        if ( XMLUtil::ToLongLong( t, llval ) ) {
            return XML_SUCCESS;
        }
        return XML_CAN_NOT_CONVERT_TEXT;
    }
...
XMLError XMLElement::QueryULongLongText( unsigned long long * ullval ) const
{
    if ( FirstChild() && FirstChild()->ToText() ) {
        const char* t = FirstChild()->ToText()->Value();
        if ( XMLUtil::ToULongLong( t, ullval ) ) {
            return XML_SUCCESS;
        }
        return XML_CAN_NOT_CONVERT_TEXT;
    }
