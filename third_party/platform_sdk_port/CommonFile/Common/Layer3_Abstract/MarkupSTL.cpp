...
/*                                                                      */
/************************************************************************/
bool CMarkupSTL::SetDoc(const char* szDoc, unsigned int strLen)
{
	// Set document text
	if ( szDoc )
	{
		m_strDoc.assign(szDoc, strLen); // = string();
		//m_strDoc = szDoc;
	}
	else
	{
...
bool CMarkupSTL::ReadTextFile( const char* szFileName, string& strDoc, string* pstrError, int* pnFlags )
{
	...
	if ( nFileByteLen )
	{
		...
		int ret = fread( pszBuffer, nFileByteLen, 1, fp );
		ret=ret;
...
bool CMarkupSTL::FindElemS( const char* szName )
{
	ResetMainPos();
	return FindElem(szName);
}

bool CMarkupSTL::FindIndexElem( const char* szName, int nIndex )
{
	// Change current position only if found
	int curPos = 0;

	while( FindElem(szName) )
	{
		if (curPos == nIndex)
		{
			return true;
...
std::string CMarkupSTL::GetAttribBase64( const char* szAttrib )
{
	std::string str=GetAttrib(szAttrib);

	const char *pTemp=str.c_str();
	if(pTemp[0] != 0)
	{
		char *pDest=new char[strlen(pTemp)];
		m_Base64.base64decone(pTemp,pDest);
		str=pDest;
		delete []pDest;
		return str;
	}
...
std::string CMarkupSTL::GetDataBase64()
{
...
bool CMarkupSTL::AddElemBase64( const char* szName, const char* szData,int nFlags/*=0*/)
{
	char *pTempBuf=new char[strlen(szData)*4/3+4+4];
	strcpy(pTempBuf,szData);
	m_Base64.b64encode((BYTE*)pTempBuf);
	bool ret=AddElem(szName,pTempBuf);
	delete []pTempBuf;
	return ret;
}

bool CMarkupSTL::SetAttribBase64( const char* szAttrib, const char* szValue )
{
	char *pTempBuf=new char[strlen(szValue)*4/3+4+4];
	strcpy(pTempBuf,szValue);
	m_Base64.b64encode((BYTE*)pTempBuf);
	bool ret=SetAttrib(szAttrib,pTempBuf);
	delete []pTempBuf;
	return ret;
}

bool CMarkupSTL::SetDataBase64( const char* szData, int nFlags/*=0*/)
{
	char *pTempBuf=new char[strlen(szData)*4/3+4+4];
	strcpy(pTempBuf,szData);
	m_Base64.b64encode((BYTE*)pTempBuf);
	bool ret=SetData(pTempBuf,nFlags);
	delete []pTempBuf;
	return ret;
}
	...
	while ( 1 )
	{
		...
		else if ( nParseFlags )
		{
			...
			else if ( nParseFlags & PD_OPENTAG )
			{
				...
				// 加入cD < 0是为支持中文
				if ( cD < 0 || cD > 0x60 || ( cD > 0x40 && cD < 0x5b ) || cD == 0x5f || cD == 0x3a )
...
bool CMarkupSTL::x_AddElem( const char* szName, tuint32 nValue, int nFlags )
{
	// Convert integer to string
	char szVal[25];
	sprintf( szVal, "%d", nValue );
...
bool CMarkupSTL::x_AddElem( const char* szName, ULONGLONG nValue, int nFlags )
{
	// Convert integer to string
	char szVal[100];
#ifdef WIN32
	sprintf(szVal,"%I64d",nValue);
#else
	sprintf(szVal,"%lld",nValue);
#endif
	return x_AddElem( szName, szVal, nFlags );
}
...
CMarkupSTL::CBase64Coder::CBase64Coder()
{
...
void CMarkupSTL::CBase64Coder::doNumConvert(long num,char *rStr,int *strLen,long *x)
{
	  if((*x=num/10))
...
int CMarkupSTL::CBase64Coder::convertToString(double num,char *rStr)
{
	  int    strLen=0;
	  long   x;
...
char *CMarkupSTL::CBase64Coder::numStr(double num,char *dest)
{
	convertToString(num,dest);
	return dest;
}

int CMarkupSTL::CBase64Coder::utf_8_encoded_size(unsigned char ch)
{
...
int CMarkupSTL::CBase64Coder::utf_8_get_code_point(unsigned char *ch,unsigned int *cp)
{
...
unsigned int CMarkupSTL::CBase64Coder::utf_8_code_point(unsigned char *ch)
{
...
char *CMarkupSTL::CBase64Coder::utf_8_uri_encoded(char *dest,char *src,char *prefix,char *suffix)
{
	int				len;
	int				srcIndex=0;
	tuint32	ch;
...
char CMarkupSTL::CBase64Coder::encode(unsigned char u)
{
 ...
 */
void CMarkupSTL::CBase64Coder::b64encode(unsigned char *src)
{
...
int	CMarkupSTL::CBase64Coder:: base64decone(const char *pInput, char *pOutput)
{
	int i = 0;
	int iCnt = 0;
