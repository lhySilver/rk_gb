...
public:
	...
	CMarkupSTL(const char *szDoc, unsigned int strLen) {SetDoc(szDoc, strLen); InitDocFlags();}
	...
	bool SetDoc( const char* szDoc, unsigned int strLen);
	bool IsWellFormed();
	bool FindElem( const char* szName=NULL );
	bool FindElemS( const char* szName=NULL ); //从头开始查找
	bool FindChildElem( const char* szName=NULL );
	bool FindIndexElem( const char* szName, int nIndex );
	bool IntoElem();
	bool OutOfElem();
	...
	std::string GetAttribBase64( const char* szAttrib );
	...
	bool AddElemBase64( const char* szName, const char* szData,int nFlags=0);
	...
	bool AddElem( const char* szName, tuint32 nValue, int nFlags=0 ) { return x_AddElem(szName,nValue,nFlags); };
	bool AddElem( const char* szName, ULONGLONG nValue, int nFlags=0 ) { return x_AddElem(szName,nValue,nFlags); };
	...
	bool RemoveNode();
	bool SetAttribBase64( const char* szAttrib, const char* szValue );
	...
	bool SetDataBase64( const char* szData, int nFlags=0 );
...
protected:
    ...
    //bool x_AddElem( const char* szName, ULONG nValue, int nFlags );
	bool x_AddElem( const char* szName, tuint32 nValue, int nFlags );
	bool x_AddElem( const char* szName, ULONGLONG nValue, int nFlags );
