...
{	
	...
	while(getline(line,fp) > 0){
		...
		//去掉utf8文件的bom头ffffffef, ffffffbb, ffffffbf
		if (firstTime)
		{
			firstTime = false;
			int line0 = line[0]&0xff;
			int line1 = line[1]&0xff;
			int line2 = line[2]&0xff;
			if(line.length() >3 &&( line0 == 0xef) &&(line1 == 0xbb) &&(line2 == 0xbf) )
			{
				line = line.substr(3);
			}
		}
...
int IniFile::getValuses(const string &section,map<string,string> &values)
{
	values.clear();
	IniSection * sect = getSection(section);
...
int IniFile::loadstring(const string &fileStr)
{	
	release();
	IniSection *section = NULL;
	unsigned int length = fileStr.length();
	if(length == 0 ){
		return -1;
	}
	...
	section = new IniSection();
	sections_[""] = section;
	unsigned int curUse = 0;
	while(curUse < length)
	{
		string temp = fileStr.substr(curUse);
		int index = temp.find_first_of('\n');
		if(index == 0)//找到的是换行，往下找一个
		{
			curUse+=1;
			continue;
		}
		else if(index < 0)//没有找到，那算最后一行
		{
			line = fileStr.substr(curUse, length-curUse);
			curUse = length;
		}
		else//找到
		{
			line = fileStr.substr(curUse, index);
			curUse+=index;
		}
		if (index != 0)
		{
