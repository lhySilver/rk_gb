...
struct IniSection
{
	...
	iterator erase(IniSection::iterator it) {return items.erase(it);}
	string name;
	string comment;
...
public:
	...
	int loadstring(const string &fileStr);
	/*将内容保存到当前文件*/
	int save();
	/*将内容另存到一个名为fname的文件*/
	...
	/*获取section段所有键值对map*/
	int getValuses(const string &section,map<string,string> &values);
