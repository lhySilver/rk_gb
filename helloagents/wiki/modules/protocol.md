...
- `GetGbRegisterConfig()` / `SetGbRegisterConfig()` 现在也直接以 `/userdata/conf/Config/gb28181.ini` 为准；即使 `ProtocolManager::Init()` 之前被外部模块调用，也会先按默认值补齐配置文件再返回结果。
- `SetGbRegisterConfig()` 只覆盖 issue38 约定的 6 个外部可编辑字段：`enabled`、`username`、`server_ip`、`server_port`、`device_id`、`password`；`device_name`、`expires_sec` 等其余注册参数改为直接沿用代码默认值。
- 运行中调用 `SetGbRegisterConfig()` 后，不再只是修改内存副本，而是会先落盘再按差异重载 GB 注册生命周期，使启停、平台地址、接入编码和设备鉴权信息立即生效。
