...
- `LocalConfigProvider` 是本地 GB/Zero/GAT 配置文件的唯一读写边界；主程序不再额外读取 `zero_config.ini` 并写回同一文件。
- `LocalConfigProvider` 的 GB/Zero/GAT 三组 INI 写入保持字段逐行显式 `fprintf`，只保留 `FinishConfigFileWrite()` 复用 `fflush/fclose` 收尾；零配置缺失错误码、模式判断和注册模式归一化直接使用常量/已有谓词/协议 normalizer，不再保留一行转发包装。默认配置初始化复用 `ProtocolExternalConfig/GbRegisterParam/GatRegisterParam` 构造函数，再覆盖本地差异项；GAT 配置更新直接校验并保存新值，不再先读旧文件后立即覆盖。本轮未新增编译单元，也未改动 Makefile/CMake。
...
- `ProtocolManager::GetGbOnlineStatus()` / `GetGatOnlineStatus()` 当前分别对外暴露 GB28181 与 GAT1400 在线态；两者都以“是否已注册到平台”为准，不把后台线程已启动但尚未注册视为在线。
- `ProtocolManager` 当前只保留 `NotifyGatFaces()` / `NotifyGatMotorVehicles()` / `NotifyGatNonMotorVehicles()` 3 个 1400 对外上报入口，供外部模块直接上报人脸、机动车、非机动车；issue49 起这 3 个入口统一改为非阻塞异步入队，已注册时立即后台发送，未注册时等待注册恢复后回放，且单条最多总发送 2 次。旧的 `NotifyGatAlarm()`、keepalive demo 和 `GAT1400CaptureEvent/GAT1400CaptureControl` 抓拍桥接链路均已移除。
