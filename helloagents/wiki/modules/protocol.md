...
- `ProtocolManager::NotifyGbAlarm()` 现在会直接向 `GB28181ClientSDK` 读取当前 `local_code` 作为报警 `DeviceID/AlarmID` 的优先来源，不再额外缓存订阅回调里的 `gbCode`。这样报警 XML 会和设备当前真实连接的 SIP 本端 ID 保持一致；零配置 `302` 之后，SDK 一旦切到正式平台 `deviceId`，报警上报也会随之切换。
