...
## 关键架构约束
- 协议配置依赖本地 HTTP 配置服务，但 GB28181 / GAT1400 注册核心参数允许通过 `/userdata/conf/Config/GB/gb28181.ini`、`/userdata/conf/Config/GB/zero_config.ini` 与 `/userdata/conf/Config/GB/gat1400.ini` 落盘并在配置服务不可达时回退使用；当前不再兼容旧的 `/userdata/conf/Config/gb28181.ini`。
