...
sequenceDiagram
    ...
    participant CFG as LocalConfigProvider
...
- `App/Protocol/ProtocolManager.*` 负责协议统一编排、通过 `LocalConfigProvider` 加载本地配置、并管理会话生命周期
...
## 关键架构约束
- 协议配置当前只保留本地配置提供者链路；GB28181 / GAT1400 注册核心参数通过 `/userdata/conf/Config/GB/gb28181.ini`、`/userdata/conf/Config/GB/zero_config.ini` 与 `/userdata/conf/Config/GB/gat1400.ini` 落盘读取；当前不再兼容旧的 `/userdata/conf/Config/gb28181.ini`。
...
| ADR-20260320-issue31 | GB28181 接入参数改为本地文件优先并支持禁用开关 | 2026-03-20 | ✅已采纳 | LocalConfigProvider, ProtocolManager | [202603201017_issue31_gb_local_config](../history/2026-03/202603201017_issue31_gb_local_config/how.md) |
