...
#ifdef WIN32
#	include <WTypes.h>
#	define STREAM_PARSER_API extern "C"
...
//最大图像分辨率
#define MAX_FRAME_WIDTH		2592
#define MAX_FRAME_HEIGTH	1944
...
/// \return 0 创建成功
/// \return < 0 创建失败
STREAM_PARSER_API HANDLE CALL_METHOD H264_PARSER_Create(int iStreamType);
...
/// \return 0 销毁成功
/// \return < 0 销毁失败
STREAM_PARSER_API int CALL_METHOD H264_PARSER_Destroy(HANDLE lHandle);
...
/// \return 0 输入成功
/// \return < 0 输入失败
STREAM_PARSER_API int CALL_METHOD H264_PARSER_InputData(HANDLE lHandle, unsigned char *pData, int nDataLen);
...
/// \return 0 复位成功
/// \return < 0 复位失败
STREAM_PARSER_API int CALL_METHOD H264_PARSER_Reset(HANDLE lHandle, int resetFlag, int streamtype);	


/// 得到下一个有效的帧数据
/// \param [in] lHandle 要操作的码流分析器的句柄
/// \return H264帧数据信息
STREAM_PARSER_API H264_FRAME_INFO * CALL_METHOD H264_PARSER_GetNextFrame(HANDLE lHandle);


/// 得到下一个有效的关键帧数据
/// \param [in] lHandle 要操作的码流分析器的句柄
/// \return H264帧数据信息
STREAM_PARSER_API H264_FRAME_INFO * CALL_METHOD H264_PARSER_GetNextKeyFrame(HANDLE lHandle);


/// 得到当前的码流类型，StreamType
/// \param [in] lHandle 要操作的码流分析器的句柄
/// \return 当前的码流类型
STREAM_PARSER_API int CALL_METHOD H264_PARSER_GetStreamType(HANDLE lHandle);
