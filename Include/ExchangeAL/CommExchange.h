...
}MotionDetectTimePoint;
typedef struct
{
	MotionDetectTimePoint begin;
	MotionDetectTimePoint end;
}MotionDetectTimePeriod;
...
struct MotionDetectConfig
{
	bool bEnable;							// 动态检测开启 
	int iLevel;								// 灵敏度: 1~6
	int iTimerCount; 						// 时间段数
	MotionDetectTimePeriod vTimePeriod[3]; 	// 有效时间段
	bool bRegionEnable;                     // 区域开启
	uint mRegion[MD_REGION_ROW];			// 区域，每一行使用一个二进制串 	
	int iInterval;                           // 报警间隔：秒
//	EventHandler hEvent;					// 动态检测联动
	bool bPersonFilterEnable;				//人形过滤开关
	bool bPersonDetectEnable;				//人形检测开关
	bool bVehicleDetectEnable;				//车辆检测开关
	bool bNonVehicleDetectEnable;			//非机动车检测开关
};

/// 全通道动态检测配置
struct MotionDetectConfigAll
{
	MotionDetectConfig vMotionDetectAll[1];
};
...
/// 人体追踪配置
struct MotionTrackConfig
{
	bool	bEnable;		///< 人体追踪开关
	bool    bPersonEnable; //人形检测开关
};
...
struct PirConfig
{
	...
	int bpir2;		///< pir开关
	int bpir3;		///< pir开关
	int iSensitivity; 	 //0-关闭 1-灵敏度低 2-灵敏度中 3-灵敏度高
	int iPirLightOnTime; //pir 触发亮灯时间 0-1分钟 1-5分钟 2-10分钟
};

/// lux
struct LuxConfig
{
	int mode; 	//Value: 0, 1, 2 0=Low，1=Middle（默认），2=High
};
...
/// 灯控配置
struct FlightWarnConfig
{
	bool bManualEnable; 	//手动开关
    bool bEnable; 		//联动开关
    int iDuration; 		//亮灯时长 5-600 sec
    int iLuminance; 		//灯亮度 1-100
    int iLuminance_yellow;   //黄灯亮度1-100
};
//Onvif配置
struct OnvifConf_S
{
	bool Enable; 			//开关
	bool FirstEnable;      //第一次开启
