...
{
	EZ_SDK_CONFIG_SNAP channel[EZ_SDK_CHANNEL_NUM];
...
typedef struct EZ_SDK_CONFIG_NETWORK
{
	...
	EZ_SDK_CONFIG_NETWORK_INTERFACE interface[EZ_SDK_NET_INTERFACE_NUM];	
...
{
	EZ_SDK_CONFIG_DHCP_ONE  interface[EZ_SDK_NET_INTERFACE_NUM];
}EZ_SDK_CONFIG_DHCP;
...
typedef struct EZ_SDK_CONFIG_DDNS_ONE
{
	...
	char	protocal[EZ_SDK_NAME_LENGTH];	//协议类型目前支持'' ," CN99 ", " Oray ", "NO-IP",
...
//回调函数原型
//设备断开连接的回调
typedef int (*EZ_SDK_DisconnectCB)(int loginID,char *devIP,int devPort,int userData);
	
//设备报警上传的回调
typedef int (*EZ_SDK_AlarmCB)(int loginID, EZ_SDK_ALARM_INFO *alarmInfo,int userData);

//实时监视数据回调
typedef int (*EZ_SDK_RealDataCB)(int playHandle,EZ_SDK_PACKET_INFO *info,char *buffer,int size,int userData);

//回放下载进度回调
typedef int (*EZ_SDK_ProgressCB)(int playHandle,int totalSize,int downloadSize,int userData);
