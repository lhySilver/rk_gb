...
#include "include/sdk_inc/tuya_ipc_video_msg.h"
...
typedef void (*TIMER_CB)(sigval_t arg);
...
void get_snapshot(char *snap_addr, int *snap_size)
{
	...
	//if(jpeg_snap_mux == 0)
	//{
	//	printf("************get snap jpeg**********\n");
	//	*snap_size=CaptureSnapshotGetBuffer(&snap_addr, 1);
	//	if( *snap_size <= 0 )
	//	{
	//		printf("snapshot err!!!\n");
	//		return ;
	...
	int ret = CaptureSnapshotGetBuffer(snap_addr, 100*1024,5);
    printf("size=%d\n",ret);
	if( ret <= 0 )
	{
		printf("snapshot err!!!\n");
    ...
    *snap_size = ret;
    ...
    char snapfile[128] = {0};
    *snap_size = 0;
  //  char s_raw_path[64] = "/mnt";
    printf("get one motion snapshot\n");
    snprintf(snapfile,64,"/tmp/demo.jpg");
	printf("path ******** is %s\n",snapfile);
...
{
	printf("******************************\n");
	tuya_ipc_cancel_doorbell_event(action);
}
#ifdef __cplusplus
}
#endif
