...
#include "include/sdk_inc/gw_intf.h"
#include "include/sdk_inc/mqc_app.h"
#include "include/sdk_inc/iot_httpc.h"
#include "include/sdk_inc/ipc_httpc.h"
#include "include/sdk_inc/app_agent.h"
#include "include/sdk_inc/mem_pool.h"
#include "include/sdk_inc/uni_thread.h"
#include "include/sdk_inc/uni_log.h"

#include "include/sdk_inc/tuya_iot_internal_api.h"
#include "include/sdk_inc/tuya_iot_base_api.h"
#include "include/sdk_inc/tuya_iot_com_api.h"
#include "include/sdk_inc/tuya_iot_wifi_api.h"
#include "include/sdk_inc/com_mmod.h"
    ...
    for( ringbuffer_stream_type = E_IPC_STREAM_VIDEO_MAIN; ringbuffer_stream_type < E_IPC_STREAM_MAX; ringbuffer_stream_type++ )
    {
        ...
        if(pMediaInfo->channel_enable[ringbuffer_stream_type] == TRUE)
        {
            if(ringbuffer_stream_type == E_IPC_STREAM_AUDIO_MAIN)
            {
                ...
                ret = tuya_ipc_ring_buffer_init(0,0,(IPC_STREAM_E)ringbuffer_stream_type,&param);
            }
            else
            {
...
OPERATE_RET tuya_ipc_sdk_ring_buffer_destroy(void)
{
	tuya_ipc_ring_buffer_uninit(0,0,E_IPC_STREAM_VIDEO_MAIN);
	tuya_ipc_ring_buffer_uninit(0,0,E_IPC_STREAM_VIDEO_SUB);
	tuya_ipc_ring_buffer_uninit(0,0,E_IPC_STREAM_AUDIO_MAIN);
    return 0;
}

STATIC VOID tuya_ipc_sdk_audio_rec_cb(IN CONST TRANSFER_AUDIO_FRAME_S *p_audio_frame, IN CONST TRANSFER_SOURCE_TYPE_E src_type)
{
	//PR_DEBUG("rev audio cb\n");
	if(s_ipc_sdk_run_handler.sdk_run_info.p2p_info.rev_audio_cb)
	{
	    s_ipc_sdk_run_handler.sdk_run_info.p2p_info.rev_audio_cb(p_audio_frame,src_type);
	}
	return;
}
...
#endif
STATIC INT_T tuya_ipc_sdk_p2p_event_proc_cb(IN CONST TRANSFER_EVENT_E event, IN CONST PVOID_T args)
{
    PR_DEBUG("p2p rev event cb=[%d] ", event);
    if(s_ipc_sdk_run_handler.sdk_run_info.p2p_info.transfer_event_cb)
    {
        return s_ipc_sdk_run_handler.sdk_run_info.p2p_info.transfer_event_cb(event, args);
    }
	return -1;
}
...
STATIC VOID *tuya_ipc_sdk_mqtt_online_proc(PVOID_T arg)
{
    ...
    while ((s_mqtt_online_status == FALSE) && (false == g_bTuyaApMode))
    {
    ...
    do
    {
		...
		if(ret != OPRT_OK)
			sleep(1);
    } while((ret != OPRT_OK) && (false == g_bTuyaApMode));
	...
	int skills = TUYA_IPC_SKILL_BASIC | TUYA_IPC_SKILL_DOWNLOAD | TUYA_IPC_SKILL_SPEED_PLAY_0Point5 | 
				TUYA_IPC_SKILL_SPEED_PLAY_2 | TUYA_IPC_SKILL_DELETE_BY_DAY;
	tuya_ipc_skill_param_u skill_param = {.value = skills};

	ret = tuya_ipc_skill_enable(TUYA_IPC_SKILL_LOCALSTG, &skill_param); 	//填充能力到skill模块
	printf("tuya_ipc_skill_enable ---> %d\n", ret);
    tuya_ipc_upload_skills(); 	//上报skill模块的数据

	if (true == g_bTuyaApMode)
	{
		OPERATE_RET ret = tuya_devos_netlink_monitor_disable();
		printf("tuya_devos_netlink_monitor_disable. ret: %d\n", ret);
		ret = lan_pro_cntl_cfg(CFG_DEFAULT_LINKAGE, NULL);	
		printf("lan_pro_cntl_cfg. ret: %d\n", ret);
	}
...
STATIC VOID tuya_ipc_sdk_raw_dp_cmd_proc(IN CONST TY_RECV_RAW_DP_S * raw_dp)
{
...
STATIC VOID tuya_ipc_sdk_dp_query(IN CONST TY_DP_QUERY_S *dp_query)
{
	if(dp_query && s_ipc_sdk_run_handler.sdk_run_info.dp_info.dp_query)
	{
		...
		s_ipc_sdk_run_handler.sdk_run_info.dp_info.dp_query((TY_DP_QUERY_S *)dp_query);
	}
}
#if defined(TUYA_EXTEND_DEVELOP)
...
#endif
INT_T tuya_ipc_sdk_sub_device_upgrade_proc_cb(IN CONST FW_UG_S *fw)
{
    ...
    {
        return s_ipc_sdk_run_handler.sdk_run_info.upgrade_info.upgrade_cb(fw);
    }
    return 0;
}
...
STATIC VOID * tuya_ipc_ap_mode_start_proc(VOID * args)
{
	printf("tuya_ipc_ap_mode_start_proc-------------->\n");
    if(FALSE == s_ipc_sdk_run_handler.sdk_run_info.quick_start_info.enable)
    {
        tuya_ipc_sdk_p2p_init();
    }
...
OPERATE_RET tuya_ipc_sdk_start(IN CONST TUYA_IPC_SDK_RUN_VAR_S * pRunInfo)
{
    ...
    //TODO loglevel
    printf("pRunInfo->debug_info.log_level : %d\n", pRunInfo->debug_info.log_level);
	...
	//setup 3:
    printf("pRunInfo->net_info.connect_mode : %d\n", pRunInfo->net_info.connect_mode);
    ...
    if (/*false == g_bTuyaApMode*/true)
    {
		...
		pthread_detach(s_ipc_sdk_run_handler.mqtt_status_change_handle);
    }
	else
	{
        //AP模式时无需等待mqtt上线，且存储功能只创建本地存储
        //TODO pthread attr set
