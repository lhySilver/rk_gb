...
requestIframeCB: call back function to request one I frame from video decoder. set to NULL if not needed or for NON-video channel.
*/
...
/* set the max vidoe afterwards of the first frame got by tuya_ipc_ring_buffer_get_video_frame, default 3s */
...
*/
OPERATE_RET tuya_ipc_ring_buffer_append_data(Ring_Buffer_User_Handle_S handle, UCHAR_T *addr, UINT_T size, MEDIA_FRAME_TYPE_E type, UINT64_T pts);

OPERATE_RET tuya_ipc_ring_buffer_append_data_with_timestamp(Ring_Buffer_User_Handle_S handle, UCHAR_T *addr, UINT_T size, MEDIA_FRAME_TYPE_E type, UINT64_T pts, UINT64_T timestamp);
...
*/
OPERATE_RET tuya_ipc_ring_buffer_append_extra_data(Ring_Buffer_User_Handle_S handle, UCHAR_T *dataAddr, UINT_T dataSize);

/* get video/audio frame from ring buffer. 
different user of the same channel device can be independently mantain status with different userIndex,
...
*/
Ring_Buffer_Node_S *tuya_ipc_ring_buffer_get_frame(Ring_Buffer_User_Handle_S handle, BOOL_T isRetry);

Ring_Buffer_Node_S* tuya_ipc_ring_buffer_get_av_frame(Ring_Buffer_User_Handle_S v_handle, Ring_Buffer_User_Handle_S a_handle, BOOL_T isRetry);
...
*/
Ring_Buffer_Node_S *tuya_ipc_ring_buffer_find_pre_by_frame(Ring_Buffer_User_Handle_S handle, UINT_T frameNum);

/* start with the newest frame, find previous frame by count frameNum, and update&anchor user(of userIndex) to this frame node.
for video channel(device), it will keep find previous frame until it's I frame.
*/
Ring_Buffer_Node_S *tuya_ipc_ring_buffer_get_pre_frame(Ring_Buffer_User_Handle_S handle, UINT_T frameNum);

/* anchor user of userIndex to specified node.
normally this API is called after tuya_ipc_ring_buffer_find_pre_video_by_frame
*/
VOID tuya_ipc_ring_buffer_anchor_user_to_node(Ring_Buffer_User_Handle_S handle, Ring_Buffer_Node_S *node);
...
*/
Ring_Buffer_Node_S *tuya_ipc_ring_buffer_get_next_target_frame(Ring_Buffer_User_Handle_S handle, UINT_T seqNo, MEDIA_FRAME_TYPE_E type);
...
*/
INT_T tuya_ipc_ring_buffer_set_max_frame_size(INT_T device, INT_T channel, IPC_STREAM_E stream, UINT_T maxFrameSize);
...
*/
OPERATE_RET tuya_ipc_ring_buffer_sync_stream(Ring_Buffer_User_Handle_S handle_video, Ring_Buffer_User_Handle_S handle_audio);

/*
    set pre-record frame position, and then just get frame
    must confirm return OPRT_OK
*/
OPERATE_RET tuya_ipc_ring_buffer_anchor_user(Ring_Buffer_User_Handle_S handle, UINT_T frameNum, BOOL_T checkOverlap);

OPERATE_RET tuya_ipc_ring_buffer_hook_mem(RING_BUFFER_MALLOC_FUNC malloc_func, RING_BUFFER_FREE_FUNC free_func);
 ...
 * @param frameIntervalMs frame interval in ms
 * @param correctAll whether correct all the timestamp, if set to false, only frames of wrong timestamp will be corrected
 * @return error code
 * - OPRT_OK success
 * - Others fail
 */
