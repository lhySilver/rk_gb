			...
			for (int k = day_begin; k <= day_end && k <= 31; k++)
			{
				...
				for (int n = 0; n < file_num; n++)
				{
					...
					if (0 == listRecordTimeInfo.size())
					{
						...
						stRecordTime.iStartTime = s_astSchRcdFileInfo2[n].iStartTime;
						stRecordTime.iEndTime = s_astSchRcdFileInfo2[n].iEndTime;
					...
					else
					{
						...
						it--;
						if (s_astSchRcdFileInfo2[n].iStartTime > (it->iEndTime+1) )
						{
							record_file_time_s stRecordTime;
							stRecordTime.iStartTime = s_astSchRcdFileInfo2[n].iStartTime;
							stRecordTime.iEndTime = s_astSchRcdFileInfo2[n].iEndTime;
...
Int32 CStorageManager::SearchRecord(Int32 year, Int32 month, Int32 day, TRecordFileTimeList &listRecordTimeInfo)
{
	...
	for (int i = 0; i < file_num; i++)
	{
		...
		{
			record_file_time_s stRecordTime;
			stRecordTime.iStartTime = s_astSchRcdFileInfo[i].iStartTime;
			stRecordTime.iEndTime = s_astSchRcdFileInfo[i].iEndTime;
			listRecordTimeInfo.push_back(stRecordTime);
		}
		else
		{
			TRecordFileTimeList::iterator it = listRecordTimeInfo.end();
			it--;
			if (s_astSchRcdFileInfo[i].iStartTime > (it->iEndTime+1) )
			{
				record_file_time_s stRecordTime;
				stRecordTime.iStartTime = s_astSchRcdFileInfo[i].iStartTime;
				stRecordTime.iEndTime = s_astSchRcdFileInfo[i].iEndTime;
