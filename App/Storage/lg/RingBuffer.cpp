...
int CRingBuffer::GetData(stream_data_header_S *pStreamDataHeader, char *pData, int max_buff_size, bool bThrow/* = false*/)
{
	int ret;
	ret = pthread_mutex_lock(&m_mutex);
	if (0 != ret)
	...
	if ( iFrameDataLen > max_buff_size )
	{
		...
		if (bThrow)
		{
			printf("<shang>------CRingBuffer::GetData throw.\n");
			//偏移读指针
			m_iReadPos += sizeof(ring_buffer_data_header_S);
			m_iReadPos %= m_iBufferLen;
			m_iReadPos += iFrameDataLen;
			m_iReadPos %= m_iBufferLen;
