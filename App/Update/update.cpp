...
#define DG_UPDATE_PACKAGE_HW_PLATFORM 		"rv1126"
#define DG_UPDATE_PACKAGE_MAGIC				(0xABCD1234)

typedef enum{
	DG_UPGRADE_IMAGE_TYPE_UBOOT 		= 1,
	DG_UPGRADE_IMAGE_TYPE_KERNEL 		= 2,
	DG_UPGRADE_IMAGE_TYPE_ROOTFS 		= 3,
	DG_UPGRADE_IMAGE_TYPE_OEM           = 4,
...
static int DG_do_update(int pkg_fd,int *flag)
{
	int result = -1;
	DG_UPDATE_IMAGE_HEAD_S image_hdr;
	...
	*flag = 0;
	...
	while( 1 )
	{
		...
		printf("check image_type : %d\n", image_type);
		printf("check image_size : %d\n", image_size);
		printf("check start_addr : %x\n", start_addr);
...
int main(int argc, char **argv)
{
    ...
    while(1)
    {
        if(access("/tmp/ota_upgrade_flag", F_OK) == 0)
        {
            ...
            START_PROCESS("sh", "sh", "-c", "rm /tmp/ota_upgrade_flag -rf", NULL);

            printf("daemon killall dgiot...\n");
			START_PROCESS("sh", "sh", "-c", "killall dgiot", NULL);
            START_PROCESS("sh", "sh", "-c", "killall wpa_supplicant", NULL);
            START_PROCESS("sh", "sh", "-c", "killall udhcpc", NULL);
            sleep(1);
			DG_update();
			START_PROCESS("sh", "sh", "-c", "reboot -f", NULL);
            return 0;
        }
