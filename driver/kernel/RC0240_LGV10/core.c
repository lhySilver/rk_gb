...
#define CONFIG_SPI_RK_SWP

#ifdef CONFIG_SPI_RK_SWP
//#define XM_WP_TEST
static int wp_lower = 0;
int common_swp_enable(struct spi_nor *nor);
int common_swp_disable(struct spi_nor *nor);
#endif
/**
...
#ifdef CONFIG_SPI_RK_SWP
	#ifdef XM_WP_TEST
	dev_err(nor->dev, "at 0x%llx, len %lld\n", (long long)instr->addr,
			(long long)instr->len);
	#endif

	if (addr > 0xEFFFFF) //15MB
	{
		wp_lower = 1;
	}
	else
...
#ifdef CONFIG_SPI_RK_SWP
	#ifdef XM_WP_TEST
	dev_err(nor->dev, "to 0x%08x, len %zd\n", (u32)to, len);
	#endif
...
static int xm_spi_nor_read_sr(struct spi_nor *nor, u8 *sr)
{
	int ret;

	if (nor->spimem) {
		struct spi_mem_op op =
			SPI_MEM_OP(SPI_MEM_OP_CMD(0x05, 1),
			SPI_MEM_OP_NO_ADDR,
			SPI_MEM_OP_NO_DUMMY,
			SPI_MEM_OP_DATA_IN(1, sr, 1));
		...
		ret = nor->controller_ops->read_reg(nor, 0x05,sr, 1);
	}
...
static int xm_spi_nor_read_sr2(struct spi_nor *nor, u8 *sr2)
{
	int ret;

	if (nor->spimem) {
		struct spi_mem_op op =
			SPI_MEM_OP(SPI_MEM_OP_CMD(0x35, 1),
			SPI_MEM_OP_NO_ADDR,
			SPI_MEM_OP_NO_DUMMY,
			SPI_MEM_OP_DATA_IN(1, sr2, 1));
		...
		ret = nor->controller_ops->read_reg(nor, 0x35,sr2, 1);
	}
...
static int xm_spi_nor_write_sr(struct spi_nor *nor, const u8 *sr, size_t len)
{
	int ret;
	...
	if (nor->spimem) {
		struct spi_mem_op op =
			SPI_MEM_OP(SPI_MEM_OP_CMD(0x01, 1),
			SPI_MEM_OP_NO_ADDR,
			SPI_MEM_OP_NO_DUMMY,
			SPI_MEM_OP_DATA_OUT(len, sr, 1));
		...
		ret = nor->controller_ops->write_reg(nor, 0x01,sr, len);
	}
...
static int xm_spi_nor_write_sr2(struct spi_nor *nor, const u8 *sr2)
{
	int ret;
	...
	if (nor->spimem) {
		struct spi_mem_op op =
			SPI_MEM_OP(SPI_MEM_OP_CMD(0x31, 1),
			SPI_MEM_OP_NO_ADDR,
			SPI_MEM_OP_NO_DUMMY,
			SPI_MEM_OP_DATA_OUT(1, sr2, 1));
		...
		ret = nor->controller_ops->write_reg(nor, 0x31,sr2, 1);
	}
 ...
 * XMC The default setting is CMP=0.
 * Returns negative if error occurred.
 */
int common_swp_enable(struct spi_nor *nor)
{
	if (0 == strncmp(nor->info->name,"XM25QH128C",10))
	{
		int ret = -1;
		u8 val_1 = 0,val_2 = 0;
		...
		ret = xm_spi_nor_read_sr(nor,&val_1);
		if (ret != 0) {
			dev_err(nor->dev,"common_swp_enable error %d reading SR1\n", ret);
			return ret;
		}
		#ifdef XM_WP_TEST
		dev_err(nor->dev,"common_swp_enable reading SR1 0x%x\n", (int) val_1);
		#endif
		...
		val_1 = (val_1 & ~(XM_BP0 | XM_BP1 | XM_BP2 |  XM_TB | XM_SEC | XM_SRP0)) ;
		#ifdef XM_WP_TEST
		dev_err(nor->dev,"common_swp_enable writing SR1 0x%x\n", (int) val_1);
		#endif
		ret = xm_spi_nor_write_sr(nor, &val_1,sizeof(val_1));
		if (ret < 0) {
			dev_err(nor->dev,"common_swp_enable error %d writing SR1\n", (int) ret);
			return ret;
		}
		...
		ret = xm_spi_nor_read_sr2(nor,&val_2);
		if (ret != 0) {
			dev_err(nor->dev,"error %d reading SR2\n", (int) val_2);
			return ret;
		}
		#ifdef XM_WP_TEST
		dev_err(nor->dev,"common_swp_enable reading SR2 0x%x\n", (int) val_2);
		#endif
		...
		val_2 = (val_2 & ~( XM_SRP1)) | XM_CMP;
		#ifdef XM_WP_TEST
		dev_err(nor->dev,"common_swp_enable writing SR2 0x%x\n", (int) val_2);
		#endif
		ret = xm_spi_nor_write_sr2(nor, &val_2);
		if (ret < 0) {
			dev_err(nor->dev,"common_swp_enable error %d writing SR2\n", (int) ret);
			return ret;
		}
	}
	else 
	{
		...
		ret = xm_spi_nor_read_sr(nor,&val);
		if (ret != 0) {
			dev_err(nor->dev,"error %d reading SR\n", (int) val);
			return ret;
		}

		val = (val & ~( XM_TB | XM_SEC | XM_SRP0)) | XM_BP0 | XM_BP1 | XM_BP2 ; //PROTECTED ALL CMD=0
		ret = xm_spi_nor_write_sr(nor, &val,sizeof(val));
		if (ret < 0) {
			dev_err(nor->dev,"error %d writing SR\n", (int) ret);
			return ret;
		}
...
int common_swp_disable(struct spi_nor *nor)
{
	if (0 == strncmp(nor->info->name,"XM25QH128C",10))
	{
		...
		#ifdef XM_WP_TEST
		dev_err(nor->dev,"common_swp_disable wp_lower %d\n", (int) wp_lower);
		#endif
		...
		ret = xm_spi_nor_read_sr(nor,&val_1);
		if (ret != 0) {
			dev_err(nor->dev,"common_swp_disable error %d reading SR1\n", ret);
			return ret;
		}
		#ifdef XM_WP_TEST
		dev_err(nor->dev,"common_swp_disable reading SR1 0x%x\n", (int) val_1);
		#endif
		...
		if (wp_lower)
		{
			val_1 = (val_1 & ~(XM_BP2 | XM_TB | XM_SEC | XM_SRP0)) | XM_BP0 | XM_BP1; //PROTECTED ADDRESSE 000000h - EFFFFFh 15MB
			#ifdef XM_WP_TEST
			dev_err(nor->dev,"common_swp_disable aaaa writing SR1 0x%x\n", (int) val_1);
			#endif
			ret = xm_spi_nor_write_sr(nor, &val_1,sizeof(val_1));
			if (ret < 0) {
				dev_err(nor->dev,"common_swp_disable error %d writing SR1\n", (int) ret);
				return ret;
			}
		}
		else
		{
			val_1 = (val_1 & ~(XM_TB | XM_SEC | XM_SRP0)) | XM_BP0 | XM_BP1 | XM_BP2; //PROTECTED NONE
			#ifdef XM_WP_TEST
			dev_err(nor->dev,"common_swp_disable bbbb writing SR1 0x%x\n", (int) val_1);
			#endif
			ret = xm_spi_nor_write_sr(nor, &val_1,sizeof(val_1));
			if (ret < 0) {
				dev_err(nor->dev,"common_swp_disable error %d writing SR1\n", (int) ret);
		...
		val_2 = xm_spi_nor_read_sr2(nor,&val_2);
		if (ret != 0) {
			dev_err(nor->dev,"common_swp_disable error %d reading SR2\n", ret);
			return ret;
		}
		#ifdef XM_WP_TEST
		dev_err(nor->dev,"common_swp_disable reading SR2 0x%x\n", (int) val_2);
		#endif
		...
		if (wp_lower)
		{
			val_2 = (val_2 & ~(XM_SRP1)) | XM_CMP;
			#ifdef XM_WP_TEST
			dev_err(nor->dev,"common_swp_disable aaaa writing SR2 0x%x\n", (int) val_2);
			#endif
			ret = xm_spi_nor_write_sr2(nor, &val_2);
			if (ret < 0) {
				dev_err(nor->dev,"error %d writing SR2\n", (int) ret);
		...
		else
		{
			val_2 = (val_2 & ~(XM_SRP1)) | XM_CMP;
			#ifdef XM_WP_TEST
			dev_err(nor->dev,"common_swp_disable bbbb writing SR2 0x%x\n", (int) val_2);
			#endif
			ret = xm_spi_nor_write_sr2(nor, &val_2);
			if (ret < 0) {
				dev_err(nor->dev,"error %d writing SR2\n", (int) ret);
	...
	else 
	{
		...
		val = (val & ~(XM_BP0 | XM_BP1 | XM_BP2 | XM_TB | XM_SEC | XM_SRP0)); //PROTECTED NONE CMD=0
		ret = xm_spi_nor_write_sr(nor, &val,sizeof(val));
		if (ret < 0) {
			dev_err(nor->dev,"error %d writing SR\n", (int) ret);
			return ret;
