...
extern int dg_do_sd_update(void);
extern int DG_do_auto_update_from_sd(void);
int board_late_init(void)
{
	...
	#define GPIO1B1_IOMUX            0xFF538008 //[4,6]
	#define GPIO1B1_PULL             0xFF5381C4 //[2,3]
	#define GPIO1B1_IOMODE           0xFF530008 //[9]
	#define GPIO1B1_OUTPUT           0xFF530000 //[9]

	uint32_t iomux = 0x00700000;
	writel(iomux,GPIO1B1_IOMUX);
	uint32_t pull = 0x000C0004;
	writel(pull,GPIO1B1_PULL);
	uint32_t iomode =  0x02000200;
	writel(iomode,GPIO1B1_IOMODE);
	uint32_t output = 0x02000000;
	writel(output,GPIO1B1_OUTPUT);
	
	printf("update start\n");
	DG_do_auto_update_from_sd();
	printf("update end\n");
