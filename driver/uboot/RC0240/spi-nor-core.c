...
static int write_disable(struct spi_nor *nor)
{
	...
	int ret = nor->write_reg(nor, SPINOR_OP_WRDI, NULL, 0);
	if (ret != 0)
	{
		printf("write disable error %d\n",ret);
	}
	return ret;
}
...
static int spi_nor_erase(struct mtd_info *mtd, struct erase_info *instr)
{
	...
	while (len) {
		...
		if (ret)
		{
			printf("spi_nor_erase_sector error %d\n",ret);
			goto erase_err;
		}
		...
		if (ret)
		{
			printf("spi_nor_erase spi_nor_wait_till_ready error %d\n",ret);
			goto erase_err;
		}
	}
	...
	size_t *retlen, const u_char *buf)
{
	...
	for (i = 0; i < len; ) {
		...
		if (ret)
		{
			printf("spi_nor_write spi_nor_wait_till_ready error %d\n",ret);
			goto write_err;
		}
		*retlen += written;
		i += written;
...
static int xm_spi_nor_read_sr(struct spi_nor *nor, u8 *sr)
{
	...
	ret = nor->read_reg(nor, 0x05, &val, 1);
	if (ret < 0) {
		printf("error %d reading SR\n", (int)ret);
		return ret;
	}
...
static int xm_spi_nor_read_sr2(struct spi_nor *nor, u8 *sr2)
{
	...
	ret = nor->read_reg(nor, 0x35, &val, 1);
	if (ret < 0) {
		printf("error %d reading SR\n", (int)ret);
		return ret;
	}
...
static int xm_spi_nor_write_sr(struct spi_nor *nor, const u8 *sr, size_t len)
{
	...
	nor->cmd_buf[0] = *sr;
	ret = nor->write_reg(nor, 0x01, nor->cmd_buf, 1);
	if (ret)
	{
		printf("xm_spi_nor_write_sr error %d\n",ret);
		return ret;
	}
...
static int xm_spi_nor_write_sr2(struct spi_nor *nor, const u8 *sr2)
{
	...
	nor->cmd_buf[0] = *sr2;
	ret = nor->write_reg(nor, 0x31, nor->cmd_buf, 1);
	if (ret)
	{
		printf("xm_spi_nor_write_sr error %d\n",ret);
		return ret;
...
int common_swp_enable(struct spi_nor *nor)
{
	if (0 == strncmp(nor->info->name,"XM25QH128C",10))
	{
		...
		if (ret != 0) {
			printf("common_swp_enable error %d reading SR1\n", (int) val_1);
			return ret;
		}

		#ifdef XM_WP_TEST
		printf("common_swp_enable reading SR1 0x%x\n", (int) val_1);
		#endif

		tmp = (val_1 & ~( XM_TB | XM_SEC | XM_SRP0)) | XM_BP0 | XM_BP1 | XM_BP2 ; //PROTECTED ALL
		if (tmp != val_1)
		{
			val_1 = tmp;

			#ifdef XM_WP_TEST
			printf("common_swp_enable writing SR1 0x%x\n", (int) val_1);
			#endif

			ret = xm_spi_nor_write_sr(nor, &val_1,sizeof(val_1));
			if (ret < 0) {
				printf("common_swp_enable error %d writing SR1\n", (int) ret);
				return ret;
			}
		}
		...
		if (ret != 0) {
			printf("common_swp_enable error %d reading SR2\n", (int) val_2);
			return ret;
		}

		#ifdef XM_WP_TEST
		printf("common_swp_enable reading SR2 0x%x\n", (int) val_2);
		#endif

		tmp = (val_2 & ~( XM_SRP1 | XM_CMP)) ; //PROTECTED ALL CMD=0
		if (tmp != val_2)
		{
			val_2 = tmp;

			#ifdef XM_WP_TEST
			printf("common_swp_enable writing SR2 0x%x\n", (int) val_2);
			#endif

			ret = xm_spi_nor_write_sr2(nor, &val_2);
			if (ret < 0) {
				printf("common_swp_enable error %d writing SR2\n", (int) ret);
				return ret;
			}
		}
	}
...
int common_swp_disable(struct spi_nor *nor)
{
	if (0 == strncmp(nor->info->name,"XM25QH128C",10))
	{
		...
		ret = xm_spi_nor_read_sr(nor,&val_1);
		if (ret != 0) {
			printf("common_swp_disable error %d reading SR1\n", (int) val_1);
			return ret;
		}

		#ifdef XM_WP_TEST
		printf("common_swp_disable reading SR1 0x%x\n", (int) val_1);
		#endif
		
		tmp = (val_1 & ~(XM_BP0 | XM_BP1 | XM_BP2 | XM_TB | XM_SEC | XM_SRP0)); //PROTECTED NONE
		if (tmp != val_1)
		{
			val_1 = tmp;

			#ifdef XM_WP_TEST
			printf("common_swp_disable writing SR1 0x%x\n", (int) val_1);
			#endif

			ret = xm_spi_nor_write_sr(nor, &val_1,sizeof(val_1));
			if (ret < 0) {
				printf("common_swp_disable error %d writing SR1\n", (int) ret);
				return ret;
			}
		}
		...
		ret = xm_spi_nor_read_sr2(nor,&val_2);
		if (ret != 0) {
			printf("common_swp_disable error %d reading SR2\n", (int) val_2);
			return ret;
		}

		#ifdef XM_WP_TEST
		printf("common_swp_disable reading SR2 0x%x\n", (int) val_2);
		#endif

		tmp = (val_2 & ~( XM_SRP1 | XM_CMP)) ; //PROTECTED NONE CMD=0
		if (tmp != val_2)
		{
			val_2 = tmp;

			#ifdef XM_WP_TEST
			printf("common_swp_disable writing SR2 0x%x\n", (int) val_2);
			#endif

			ret = xm_spi_nor_write_sr2(nor, &val_2);
			if (ret < 0) {
				printf("common_swp_disable error %d writing SR2\n", (int) ret);
				return ret;
			}
		}
	}
