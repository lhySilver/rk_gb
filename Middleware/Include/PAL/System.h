...
///	风扇启动
///	\param	on=1 打开电扇; on=0	关闭电扇
int  Systemfan(unsigned int on);

///	获取温度值
/// param	pTemperature 温度值
/// \return 0 成功
/// \return 非0	失败
int SystemGetCurrentTemperature(int *pTemperature);
...
int SystemGetUUID(char *pUUID);

const char * SystemGetID(void);
const char * SystemGetHWID(void);
//<shang>int SystemDropCaches();

typedef struct MEM_TOP
{
	int shrd;
	int used;
	int free;
...
int SystemGetMem(MEM_TOP_T *MEM);
...
{
	STREAM_ITEM_T item[3];
}STREAM_INFO_T;

int SystemCheckStreamInfo(STREAM_INFO_T *info);
