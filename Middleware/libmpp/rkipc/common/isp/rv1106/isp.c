...
char main_scene[32] = "normal";//change on 2024.12.14
char sub_scene[32];
...
int isp_camera_group_init(int cam_group_id, rk_aiq_working_mode_t WDRMode, bool MultiCam,
	...
	if (access("/data/awb_calib.ini", F_OK) == 0) {
		...
		rk_awb_calib_init(calib_file_path);
	
		rk_aiq_camgroup_camInfos_t camInfos;
		memset(&camInfos, 0, sizeof(camInfos));
		if (rk_aiq_uapi2_camgroup_getCamInfos((rk_aiq_camgroup_ctx_t *)ctx, &camInfos) == XCAM_RETURN_NO_ERROR) {
			for (int i = 0; i < camInfos.valid_sns_num; i++) {
				rk_aiq_sys_ctx_t* aiq_ctx = NULL;
				aiq_ctx = rk_aiq_uapi2_camgroup_getAiqCtxBySnsNm((rk_aiq_camgroup_ctx_t *)ctx, camInfos.sns_ent_nm[i]);
				if (!aiq_ctx)
					continue;
				...
				if (i==0)
				{
					...
					otp_info.otp_awb.r_value = rk_awb_calib_get_int("RKAWBOTPParam:r_value", 1263);
					otp_info.otp_awb.b_value = rk_awb_calib_get_int("RKAWBOTPParam:b_value", 1263);
					otp_info.otp_awb.gr_value = rk_awb_calib_get_int("RKAWBOTPParam:gr_value", 978);
					otp_info.otp_awb.gb_value = rk_awb_calib_get_int("RKAWBOTPParam:gb_value", -1);

					printf("====> %d: r_value=%d,b_value=%d,gr_value=%d,gb_value=%d\n",
						0,otp_info.otp_awb.r_value,otp_info.otp_awb.b_value,otp_info.otp_awb.gr_value,otp_info.otp_awb.gb_value);

					otp_info.otp_awb.golden_r_value = rk_awb_calib_get_int("RKAWBOTPParam:golden_r_value", 1136);
					otp_info.otp_awb.golden_b_value = rk_awb_calib_get_int("RKAWBOTPParam:golden_b_value", 1140);
					otp_info.otp_awb.golden_gr_value = rk_awb_calib_get_int("RKAWBOTPParam:golden_gr_value", 973);
					otp_info.otp_awb.golden_gb_value = rk_awb_calib_get_int("RKAWBOTPParam:golden_gb_value", -1);

					printf("====> %d: golden_r_value=%d,golden_b_value=%d,golden_gr_value=%d,golden_gb_value=%d\n",
						1,otp_info.otp_awb.golden_r_value,otp_info.otp_awb.golden_b_value,otp_info.otp_awb.golden_gr_value,otp_info.otp_awb.golden_gb_value);
				}
				else
				{
					otp_info.otp_awb.flag = false;
				}
...
int rk_isp_set_scenario_v10(int cam_id, const char *value) 
{
	if (!strcmp(value, "day")) 
    {
		current_scenario_id = 0;
...
int rk_isp_setAntiFlicker(int cam_id,int value)
{
	//value 0-关闭 1-弱抗50hz 2-强抗50hz
	int ret = -1;
	static int cam0_vlaue = -1;
	static int cam1_vlaue = -1;
	if (0 == cam_id)
	{
		if (cam0_vlaue == value)
		{
	...
	CalibDb_AntiFlickerModeV2_t  cal_mode;
	CalibDb_FlickerFreqV2_t  Frequency;
	bool enable = false;
	rk_aiq_sys_ctx_t* ctx = rkipc_aiq_get_ctx(cam_id);
	if (1 == value)
	{
		enable = true;
		cal_mode = AECV2_ANTIFLICKER_NORMAL_MODE;
	...
	else if (2 == value)
	{
		enable = true;
		cal_mode = AECV2_ANTIFLICKER_NORMAL_MODE;
		Frequency = AECV2_FLICKER_FREQUENCY_60HZ;
	}
	else
	{
		enable = false;
		cal_mode = AECV2_ANTIFLICKER_AUTO_MODE;
		Frequency = AECV2_FLICKER_FREQUENCY_OFF;
	}
	...
	if (ret != 0) 
	{
		LOG_ERROR("rk_aiq_user_api2_ae_getExpSwAttr error!\n");
	}
	else
	{
		LOG_ERROR("rk_aiq_user_api2_ae_getExpSwAttr ok\n");
	}
	//set antifilicker mode
	expSwAttr.stAuto.stAntiFlicker.enable = enable;
	expSwAttr.stAuto.stAntiFlicker.Frequency = Frequency;
	expSwAttr.stAuto.stAntiFlicker.Mode = cal_mode;
	ret = rk_aiq_user_api2_ae_setExpSwAttr(ctx, expSwAttr);
	if (ret != 0) 
	{
		LOG_ERROR("rk_aiq_user_api2_ae_setExpSwAttr error!\n");
	}
	else
	{
		LOG_ERROR("rk_aiq_user_api2_ae_setExpSwAttr ok\n");
	}
...
int rk_isp_set_night_to_day_v10(int cam_id, const char *value) 
{
	int ret;
	RK_ISP_CHECK_CAMERA_ID(cam_id);
	aie_attrib_t attr;
	ret = rk_aiq_user_api2_aie_GetAttrib(rkipc_aiq_get_ctx(cam_id), &attr);
	if (!strcmp(value, "night")) 
    {
		attr.mode = RK_AIQ_IE_EFFECT_BW;
		ret = rk_aiq_user_api2_aie_SetAttrib(rkipc_aiq_get_ctx(cam_id), &attr);
		// rk_isp_enable_ircut(false);
		LOG_INFO("SET NIGHT\n");
	}
    else
...
int rk_isp_set_image_flip_ex(int cam_id, const char *value) {
	RK_ISP_CHECK_CAMERA_ID(cam_id);
	if (rkipc_aiq_use_group) {
		LOG_INFO("group mode, not support set mirror/flip\n");
		return 0;
	...
	LOG_INFO("set mirror=%d flip=%d\n",mirror,flip);
	ret = rk_aiq_uapi2_setMirrorFlip(rkipc_aiq_get_ctx(cam_id), mirror, flip, 4); // skip 4 frame
	if (ret) {
		LOG_ERROR("rk_isp_set_sharpness failed %d", ret);
		return ret;
	}
