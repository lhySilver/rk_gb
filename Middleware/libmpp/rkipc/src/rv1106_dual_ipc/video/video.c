...
static MPP_CHN_S vi_chn[2], vpss_chn[2], venc_chn[3], avs_in_chn[2], avs_out_chn[2], ivs_chn[2];
...
int rkipc_avs_init() {
	...
	if (rk_param_get_int("avs:param_source", 0)) {
		...
		const char *file = rk_param_get_string("avs:calib_file_path", "/data/avs_calib.xml");
		...
		const char *mesh_alpha_path = rk_param_get_string("avs:mesh_alpha_path", "/usr/share/avs_calib/");
	...
	stAvsGrpAttr.stOutAttr.fDistance = stitch_distance;//rk_param_get_int("avs:stitch_distance", 5);
...
int rkipc_bind_init() {
	...
	for (int i = 0; i < g_sensor_num; i++) {
		...
		ivs_chn[i].enModId = RK_ID_IVS;
		ivs_chn[i].s32DevId = 0;
		ivs_chn[i].s32ChnId = i;
	...
	if (enable_jpeg) 
	{
		...
		venc_chn[2].s32ChnId = 2;
		
		ret = RK_MPI_SYS_Bind(&avs_out_chn[1], &venc_chn[2]);
		if (ret != RK_SUCCESS) {
			LOG_ERROR("bind error %#x: avs_out_chn[1] [%d, %d] -> venc_chn[1] [%d, %d]", ret,
						avs_out_chn[1].s32DevId, avs_out_chn[1].s32ChnId, venc_chn[2].s32DevId,
						venc_chn[2].s32ChnId);
			return ret;
		}
	}
	...
	if (enable_npu)
	{
		ret = RK_MPI_SYS_Bind(&avs_out_chn[1], &vpss_chn[0]);
		if (ret != RK_SUCCESS) {
			LOG_ERROR("bind error %#x: avs_out_chn[1] [%d, %d] -> vpss_chn[0] [%d, %d]", ret,
						avs_out_chn[1].s32DevId, avs_out_chn[1].s32ChnId, vpss_chn[0].s32DevId,
						vpss_chn[0].s32ChnId);
			return ret;
		}
		pthread_create(&vpss_thread_rgb, NULL, rkipc_get_avs_vpss_bgr, NULL);
		LOG_INFO("rkipc_osd_draw_nn_init\n");
	...
	if (enable_ivs)
	{
		...
		ret = RK_MPI_SYS_Bind(&vpss_chn[0], &ivs_chn[0]);
		if (ret != RK_SUCCESS) {
			LOG_ERROR("bind error %#x: vpss_chn[0] [%d, %d] -> ivs_chn[0] [%d, %d]", ret,
						vpss_chn[0].s32DevId, vpss_chn[0].s32ChnId, ivs_chn[0].s32DevId,
						ivs_chn[0].s32ChnId);
			return ret;
		}
		#else
		ret = RK_MPI_SYS_Bind(&avs_out_chn[1], &ivs_chn[0]);
		if (ret != RK_SUCCESS) {
			LOG_ERROR("bind error %#x: avs_out_chn[1] [%d, %d] -> ivs_chn[0] [%d, %d]", ret,
						avs_out_chn[1].s32DevId, avs_out_chn[1].s32ChnId, ivs_chn[0].s32DevId,
						ivs_chn[0].s32ChnId);
			return ret;
		}
		#endif
...
int rkipc_bind_deinit() {
	...
	if (enable_ivs)
	{
		...
		if (!enable_npu)
		{
			ret = RK_MPI_SYS_UnBind(&avs_out_chn[1], &vpss_chn[0]);
			if (ret != RK_SUCCESS) 
			{
				LOG_ERROR("unbind error %#x: avs_out_chn[1] [%d, %d] -> vpss_chn[0] [%d, %d]", ret,
							avs_out_chn[1].s32DevId, avs_out_chn[1].s32ChnId, vpss_chn[0].s32DevId,
							vpss_chn[0].s32ChnId);
				return ret;
			}
		}
		ret = RK_MPI_SYS_UnBind(&vpss_chn[0], &ivs_chn[0]);
		if (ret != RK_SUCCESS) 
		{
			LOG_ERROR("unbind error %#x: vpss_chn[0] [%d, %d] -> ivs_chn[0] [%d, %d]", ret,
						vpss_chn[0].s32DevId, vpss_chn[0].s32ChnId, ivs_chn[0].s32DevId,
						ivs_chn[0].s32ChnId);
			return ret;
		}
		#else
		ret = RK_MPI_SYS_UnBind(&avs_out_chn[1], &ivs_chn[0]);
		if (ret != RK_SUCCESS) 
		{
			LOG_ERROR("unbind error %#x: avs_out_chn[1] [%d, %d] -> ivs_chn[0] [%d, %d]", ret,
						avs_out_chn[1].s32DevId, avs_out_chn[1].s32ChnId, ivs_chn[0].s32DevId,
						ivs_chn[0].s32ChnId);
			return ret;
		}
    ...
    if (enable_jpeg)
    {
        ret = RK_MPI_SYS_UnBind(&avs_out_chn[1], &venc_chn[2]);
		if (ret != RK_SUCCESS) 
		{
			LOG_ERROR("unbind error %#x: avs_out_chn[1] [%d, %d] -> venc_chn[2] [%d, %d]", ret,
						avs_out_chn[1].s32DevId, avs_out_chn[1].s32ChnId, venc_chn[2].s32DevId,
						venc_chn[2].s32ChnId);
			return ret;
		}
...
int rkipc_osd_draw_nn_init() {
	...
	{
		RGN_HANDLE RgnHandle = DRAW_NN_OSD_EX_ID;
		RGN_ATTR_S stRgnAttr;
		MPP_CHN_S stMppChn;
		RGN_CHN_ATTR_S stRgnChnAttr;
		BITMAP_S stBitmap;
...
int rk_video_set_rotation(int value) {
	...
	if (ret)
		LOG_ERROR("RK_MPI_VI_SetChnMirrorFlip ViPipe:%d ViChn:%d error! ret=%#x\n", pipe_id_,VIDEO_PIPE_0,ret);

	ret = RK_MPI_VI_SetChnMirrorFlip(pipe_id_, VIDEO_PIPE_1, stMirrFlip);
	if (ret)
		LOG_ERROR("RK_MPI_VI_SetChnMirrorFlip ViPipe:%d ViChn:%d error! ret=%#x\n", pipe_id_,VIDEO_PIPE_1,ret);

	ret = RK_MPI_VI_SetChnMirrorFlip(pipe_id_, VIDEO_PIPE_2, stMirrFlip);
	if (ret)
		LOG_ERROR("RK_MPI_VI_SetChnMirrorFlip ViPipe:%d ViChn:%d error! ret=%#x\n", pipe_id_,VIDEO_PIPE_2,ret);
...
int rk_video_init() {
	LOG_DEBUG("begin\n");
	int ret = 0;
	enable_ivs = rk_param_get_int("avs:enable_ivs", 0);
	...
	enable_rtsp = rk_param_get_int("avs:enable_rtsp", 0);
	enable_rtmp = rk_param_get_int("avs:enable_rtmp", 0);
	LOG_INFO("enable_ivs is %d, enable_jpeg is %d, enable_venc_0 is %d, enable_venc_1 is %d, enable_rtsp is %d, "
	         "enable_rtmp is %d\n",
	         enable_ivs,enable_jpeg, enable_venc_0, enable_venc_1, enable_rtsp, enable_rtmp);
...
int rk_video_deinit() {
	...
	if (enable_jpeg) {
		LOG_ERROR("rkipc_jpeg_deinit\n");
		ret |= rkipc_jpeg_deinit();
	}
	if (enable_ivs) {
		LOG_ERROR("rkipc_ivs_deinit\n");
		ret |= rkipc_ivs_deinit();
	}
	if (enable_npu) {
		LOG_ERROR("rkipc_video_det_deinit\n");
		ret |= rkipc_video_det_deinit();
	}
	if (enable_ivs || enable_npu) {
		LOG_ERROR("rkipc_vpss_0_deinit\n");
...
static void *rkipc_get_qrcode(void *arg) {
	...
	while (g_vi_qrcode_run_) {
		...
		ret = RK_MPI_VPSS_GetChnFrame(0, 0, &frame, 1000);
		if (ret == RK_SUCCESS) {
			if (QrCodeDecodeCb_g) {
			...
			ret = RK_MPI_VPSS_ReleaseChnFrame(0, 0, &frame);
...
static void *rkipc_ivs_get_results(void *arg) {
	...
	int md = 1;
	int od = 0;
	int width = rk_param_get_int("video.2:width", 1920);
	int height = rk_param_get_int("video.2:height", 1080);
	...
	while (g_ivs_run_) {
		...
		if (ret >= 0) {
			...
			// LOG_INFO("get stResults.s32ResultNum %d\n", stResults.s32ResultNum);
			if (md == 1) {
				if (resultscount % 10 == 0 && stResults.s32ResultNum == 1) {
					...
					// printf("sen=%d\n",sen);
					if (count > (x * y * 8 /sen)) {
						LOG_INFO("Detect movement count=%d > %d\n",count,x * y * 8 /sen);
						if (ivs_cb)
						{
							ivs_cb(1);
						}
					...
					{
						//LOG_INFO("111 Detect no movement\n");
						if (ivs_cb)
						{
							ivs_cb(0);
						}
				...
				// else
				// {
				// 	LOG_INFO("222 Detect no movement\n");
				// 	if (ivs_cb)
				// 	{
				// 		ivs_cb(0);
				// 	}
...
static void *rkipc_get_avs_vpss_bgr(void *arg) {
	...
	while (g_video_run_) {
		...
		if (ret == RK_SUCCESS) {
			...
			int32_t fd = RK_MPI_MB_Handle2Fd(frame.stVFrame.pMbBlk);
			rkipc_rockiva_write_nv12_frame_by_fd(frame.stVFrame.u32Width,frame.stVFrame.u32Height, loopCount, fd);
...
int rkipc_vpss_0_init()
{
	...
	stVpssChnAttr[0].u32Width = rk_param_get_int("video.2:width", 1920);
	stVpssChnAttr[0].u32Height = rk_param_get_int("video.2:height", 1080);
...
int rkipc_jpeg_init() 
{
	RK_U32 chnId = VIDEO_PIPE_2;
	RK_U32 width = rk_param_get_int("video.1:width", 1920);
	RK_U32 height = rk_param_get_int("video.1:height", 1080);
	RK_CODEC_ID_E enType = RK_VIDEO_ID_JPEG;
...
int rkipc_ivs_init()
{
	int ret;
	IVS_CHN IvsChn = 0;
	int video_width =  rk_param_get_int("video.2:width", 1920);
	int video_height = rk_param_get_int("video.2:height", 1080);
	int buf_cnt = 2;
	int smear = 0;
	int weightp = 1;
	int md = 1;
	...
	attr.s32ODPercent = 7;
	ret = RK_MPI_IVS_CreateChn (IvsChn, &attr);
	if (RK_SUCCESS != ret) 
	{
		LOG_ERROR("RK_MPI_IVS_CreateChnfailed with %#x!\n", ret);
		return ret;
	}
	LOG_INFO("RK_MPI_IVS_CreateChn success\n");

	IVS_MD_ATTR_S stMdAttr;
	memset(&stMdAttr, 0, sizeof(stMdAttr));
	ret = RK_MPI_IVS_GetMdAttr(IvsChn, &stMdAttr);
	if (ret) {
	...
	ret = RK_MPI_IVS_SetMdAttr(IvsChn, &stMdAttr);
	if (ret) {
...
int rkipc_ivs_deinit()
{
	...
	// IVS
	ret = RK_MPI_IVS_DestroyChn(IvsChn);
	if (ret)
...
{
	// bInstant暂时只?持RK_FLASE，设置RK_TRUE会返回不?持VENC错误码
	// RK_ERR_VENC_NOT_SUPPORT
	return RK_MPI_VENC_RequestIDR(stream_id,RK_FALSE);
}

int rk_video_set_vi_mirror_flip(int stream_id, unsigned char mirror, unsigned char flip)
{
	VI_PIPE ViPipe = 0;
	VI_CHN ViChn = 0;
	VI_ISP_MIRROR_FLIP_S stMirrFlip;
	...
	return RK_MPI_VI_SetChnMirrorFlip(ViPipe, ViChn, stMirrFlip);
}