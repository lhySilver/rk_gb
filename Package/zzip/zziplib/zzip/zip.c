...
#define return(val) { e=val; goto cleanup; }
	...
	/* the new offset shall overlap with the area after the old offset! */
		...
		if (mapoff < pagesize) {
		    ...
		    if (mapoff & (pagesize-1)) { /* only on first run */
...
__zzip_dir_parse (ZZIP_DIR* dir)
{
    zzip_error_t rv;
    zzip_off_t filesize;
    struct zzip_disk_trailer  __attribute__((aligned(4))) trailer;
    /* if (! dir || dir->fd < 0) 
...
ZZIP_DIR *
zzip_dir_open_from_mem(unsigned char * baseAddr, unsigned long buflen)
{
	int fd;
	zzip_error_t e;
	extern struct zzip_plugin_io mem_io	;
	
	zzip_plugin_io_t io = &mem_io;
	
	fd = luazip_open(baseAddr, buflen);
	
	return zzip_dir_fdopen_ext_io(fd, NULL, NULL, io);
}
