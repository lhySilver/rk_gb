...
template<> void exchangeTable<MotionDetectTimePoint>(CConfigTable &table, MotionDetectTimePoint &config, int state)
{
	CKeyExchange exchanger;

	exchanger.setState(state);
	exchanger.exchange(table, "Hour", config.hour);
	exchanger.exchange(table, "Minute", config.minute);
}
template<> void exchangeTable<MotionDetectTimePeriod>(CConfigTable &table, MotionDetectTimePeriod &config, int state)
{
	CConfigTable& tableTimeBegin = table["Begin"];
	exchangeTable(tableTimeBegin, config.begin, state);
	
	CConfigTable& tableTimeEnd = table["End"];
	exchangeTable(tableTimeEnd, config.end, state);
}
template<> void exchangeTable<MotionDetectConfig>(CConfigTable &table, MotionDetectConfig &config, int state)
{
	...
	exchanger.exchange(table, "TimerCount", config.iTimerCount);
    exchanger.exchange(table, "bRegionEnable", config.bRegionEnable);
    exchanger.exchange(table, "Interval", config.iInterval);
	CConfigTable& tableTimePeriod = table["TimePeriod"];
	for(int i = 0; i < 3; i++)
	{
		exchangeTable(tableTimePeriod[i], config.vTimePeriod[i], state);
	}
...
//	exchangeEventHandler(table["EventHandler"], config.hEvent, state);
	exchanger.exchange(table, "PersonFilterEnable", config.bPersonFilterEnable);
	exchanger.exchange(table, "PersonDetectEnable", config.bPersonDetectEnable);
	exchanger.exchange(table, "VehicleDetectEnable", config.bVehicleDetectEnable);
	exchanger.exchange(table, "NonVehicleDetectEnable", config.bNonVehicleDetectEnable);
}
...
template<> void exchangeTable<MotionTrackConfig>(CConfigTable &table, MotionTrackConfig &config, int state)
{
	CKeyExchange exchanger;
	
	exchanger.setState(state);
	exchanger.exchange(table, "Enable", config.bEnable);
	exchanger.exchange(table, "PersonEnable", config.bPersonEnable);
}

template<> void exchangeTable<PirConfig>(CConfigTable &table, PirConfig &config, int state)
{
	CKeyExchange exchanger;
	
	exchanger.setState(state);
	exchanger.exchange(table, "bpir1", config.bpir1);
	exchanger.exchange(table, "bpir2", config.bpir2);
	exchanger.exchange(table, "bpir3", config.bpir3);
	exchanger.exchange(table, "Sensitivity", config.iSensitivity);
	exchanger.exchange(table, "PirLightOnTime", config.iPirLightOnTime);
}
template<> void exchangeTable<LuxConfig>(CConfigTable &table, LuxConfig &config, int state)
{
	CKeyExchange exchanger;
...
template<> void exchangeTable<VoiceConfig>(CConfigTable &table, VoiceConfig &config, int state)
{
	CKeyExchange exchanger;
	
	exchanger.setState(state);
	exchanger.exchange(table, "voiceonoff", config.bVoiceonoff);
	exchanger.exchange(table, "voicesentivity", config.IVoicesentivity);
}
template<> void exchangeTable<SirenConfig>(CConfigTable &table, SirenConfig &config, int state)
{
	CKeyExchange exchanger;
	
	exchanger.setState(state);
	exchanger.exchange(table, "autosiren", config.autosiren);
    exchanger.exchange(table, "sirenlist", config.sirenlist);
    exchanger.exchange(table, "sirenduration", config.sirenduration);
	exchanger.exchange(table, "sirenvolume", config.sirenvolume);
}
template<> void exchangeTable<IpcVolConfig>(CConfigTable &table, IpcVolConfig &config, int state)
{
	CKeyExchange exchanger;
	
	exchanger.setState(state);
	exchanger.exchange(table, "ipcvolume", config.ipcvolume);
}
...
template<> void exchangeTable<PtzPosition>(CConfigTable& table, PtzPosition& config, int state)
{
	CKeyExchange exchanger;
	...
	exchanger.exchange(table, "PX", config.privatex);
	exchanger.exchange(table, "PY", config.privatey);
}

template<> void exchangeTable<FlightWarnConfig>(CConfigTable &table, FlightWarnConfig &config, int state)
{
	CKeyExchange exchanger;
	
	exchanger.setState(state);
	exchanger.exchange(table, "ManualEnable", config.bManualEnable);
	exchanger.exchange(table, "Enable", config.bEnable);
    exchanger.exchange(table, "Duration", config.iDuration);
    exchanger.exchange(table, "Luminance", config.iLuminance);
    exchanger.exchange(table, "Luminance_yellow", config.iLuminance_yellow);
}

//Onvif配置
template<> void exchangeTable<OnvifConf_S>(CConfigTable &table, OnvifConf_S &config, int state)
{
	CKeyExchange exchanger;
