...
template<> void exchangeTable<RecordConfig>(CConfigTable& table, RecordConfig& config, int state)
{
	...
	exchanger.exchange(table, "MuteRecord", config.bMuteRecord);
	exchanger.exchange(table, "PacketLength", config.iPacketLength);
	exchanger.exchange(table, "RecordEnable", config.bRecordEnable);
	exchanger.exchange(table, "RecordMode", config.iRecordMode);
